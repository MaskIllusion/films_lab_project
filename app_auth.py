...
def connect_films():
    return psycopg.connect(
        host="localhost", dbname="films_lab",
        user="postgres", password=PG_PASSWORD
    )
...
def open_films_window():
    ...
    ttk.Label(win, text="Режиссёр").pack(anchor=NW, padx=6, pady=2)
    entry_dir = ttk.Entry(win); entry_dir.pack(anchor=NW, padx=6, pady=2)

    ttk.Label(win, text="Год").pack(anchor=NW, padx=6, pady=2)
    entry_year = ttk.Entry(win); entry_year.pack(anchor=NW, padx=6, pady=2)

    ttk.Label(win, text="Минуты").pack(anchor=NW, padx=6, pady=2)
    entry_min = ttk.Entry(win); entry_min.pack(anchor=NW, padx=6, pady=2)

    ttk.Button(win, text="Добавить", command=save_film).pack(anchor=NW, padx=6, pady=6)
